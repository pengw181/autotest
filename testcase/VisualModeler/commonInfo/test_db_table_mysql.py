...
class DatabaseManagementMysql(unittest.TestCase):
	...
	def test_15_EditDBTableCol(self):
		u"""数据管理，mysql数据库，数据表修改字段，修改列名(自定义)"""
		action = {
			"操作": "EditDBTableCol",
			"参数": {
				"数据库名称": "auto_mysql数据库",
		...
		CheckMsg|保存成功
		"""
		log.info('>>>>> 数据管理，mysql数据库，数据表修改字段，修改列名(自定义) <<<<<')
		gbl.temp.set("StartTime", datetime.now().strftime('%Y%m%d%H%M%S'))
		result = self.worker.action(action)
		assert result
		log.info(gbl.temp.get("ResultMsg"))
	...
	def test_46_ImportDBTable(self):
		u"""数据管理，mysql数据库，导入表，导入超多字段，100列"""
		action = {
			"操作": "ImportDBTable",
			"参数": {
				"数据库名称": "auto_mysql数据库",
		...
		CheckMsg|操作成功
		"""
		log.info('>>>>> 数据管理，mysql数据库，导入表，导入超多字段，100列 <<<<<')
		gbl.temp.set("StartTime", datetime.now().strftime('%Y%m%d%H%M%S'))
		result = self.worker.action(action)
		assert result
		log.info(gbl.temp.get("ResultMsg"))
	...
	def test_47_ImportDBTable(self):
		u"""数据管理，mysql数据库，导入表，导入超多字段，200列"""
		action = {
			"操作": "ImportDBTable",
			"参数": {
				"数据库名称": "auto_mysql数据库",
		...
		CheckMsg|操作成功
		"""
		log.info('>>>>> 数据管理，mysql数据库，导入表，导入超多字段，200列 <<<<<')
		gbl.temp.set("StartTime", datetime.now().strftime('%Y%m%d%H%M%S'))
		result = self.worker.action(action)
		assert result
		log.info(gbl.temp.get("ResultMsg"))
