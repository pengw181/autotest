...
class WorkFlowInterfaceNodeSoap(unittest.TestCase):

	log.info("装载流程soap接口调用测试用例")
	worker = CaseWorker()
	...
	def test_10_NodeFetchConf(self):
		...
		log.info('>>>>> 节点添加取数配置，返回内容为xml，解析xml <<<<<')
		gbl.temp.set("StartTime", datetime.now().strftime('%Y%m%d%H%M%S'))
		result = self.worker.action(action)
		assert result

	def test_11_LineNode(self):
		u"""开始节点连线到节点：参数设置"""
		action = {
			"操作": "LineNode",
			"参数": {
