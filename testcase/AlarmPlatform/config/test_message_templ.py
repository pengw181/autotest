...
class MessageTemplConfig(unittest.TestCase):
	...
	def test_3_DeleteMsgTemplate(self):
		u"""当前模版为默认模版，删除模版"""
		action = {
		...
		checks = """
		CheckMsg|不允许删除默认消息模版
		"""
		log.info('>>>>> 当前模版为默认模版，删除模版 <<<<<')
		gbl.temp.set("StartTime", datetime.now().strftime('%Y%m%d%H%M%S'))
		result = self.worker.action(action)
		assert result
		log.info(gbl.temp.get("ResultMsg"))
	...
	def test_4_UpdateMsgTemplateStatus(self):
		u"""启用消息模版"""
		action = {
	...
	def test_5_UpdateMsgTemplate(self):
		u"""修改已启用的消息模版"""
		action = {
	...
	def test_6_UpdateMsgTemplateStatus(self):
		u"""禁用消息模版"""
		action = {
			"操作": "UpdateMsgTemplateStatus",
			"参数": {
	...
	def test_7_SetDefaultMsgTemplate(self):
	...
	def test_8_UpdateMsgTemplate(self):
		u"""修改消息模版，使用高级模式"""
		action = {
			"操作": "UpdateMsgTemplate",
			"参数": {
	...
	def test_9_UpdateMsgTemplateStatus(self):
		u"""启用消息模版"""
		action = {
			"操作": "UpdateMsgTemplateStatus",
			"参数": {
	...
	def test_10_AddMsgTemplate(self):
		u"""同一个告警规则下，添加多个消息模版"""
		action = {
			"操作": "AddMsgTemplate",
			"参数": {
	...
	def test_11_UpdateMsgTemplateStatus(self):
		u"""启用消息模版"""
		action = {
			"操作": "UpdateMsgTemplateStatus",
			"参数": {
	...
	def test_12_SetDefaultMsgTemplate(self):
	...
	def test_20_AddMsgTemplate(self):
	...
	def test_21_BatchEnableMsgTemplate(self):
		u"""批量启用消息模版"""
		action = {
	...
	def test_22_BatchDisableMsgTemplate(self):
		u"""批量禁用消息模版"""
		action = {
	...
	def test_23_BatchEnableMsgTemplate(self):
		u"""批量启用消息模版"""
		action = {
			"操作": "BatchEnableMsgTemplate",
			"参数": {
