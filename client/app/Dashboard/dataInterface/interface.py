...
class CustomizeInterface:
    ...
    def _choose(self, table_name):
        """
        # 选择一条表接口
        :param table_name: 表中文名称
        """
        self.browser.find_element(By.XPATH, "//*[@id='tableName']/following-sibling::span/input[1]").clear()
        self.browser.find_element(By.XPATH, "//*[@id='tableName']/following-sibling::span/input[1]").send_keys(table_name)
        log.info("表中文名称输入: {0}".format(table_name))
        self.browser.find_element(By.XPATH, "//*[@title='查询自定义数据接口列表']").click()
        sleep(2)
        self.browser.find_element(By.XPATH, "//*[@field='tableNameCH']/*[text()='{0}']".format(table_name)).click()
        log.info("选择表: {0}".format(table_name))

    def fieldConversion(self, table_name, conversion):
        """
        # 字段类型转化
        :param table_name: 表中文名称
    ...
    def fieldClassify(self, table_name, xList, yList, gList):
        """
        # 字段分类
        :param table_name: 表中文名称
        :param xList: 维度字段，数组
