...
class NodeControl:
    ...
    def _var_list_loop(self, mode, varName, outVarName, valueType, varType):
        ...
        if mode:
            self.browser.find_element(
                By.XPATH, "//*[@name='mode']/following-sibling::label[text()='{0}']".format(mode)).click()
        ...
        # 变量名称
        if varName:
            self.browser.find_element(By.XPATH, "//*[@id='dataH_inputVarControl2Name']/following-sibling::span//a").click()
        ...
        # 赋值方式
        if valueType:
            elements = self.browser.find_elements(By.XPATH, "//*[@id='valuetype_1']/following-sibling::span[1]//a")
            for e1 in elements:
                if e1.is_displayed():
                    e1.click()
