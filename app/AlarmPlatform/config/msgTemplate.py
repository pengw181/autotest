...
class MsgTemplate:
    ...
    def set_default_template(self, obj, set_default):
        ...
        if set_default == "是":
            if default_status:
                ...
                self.browser.find_element(
                    By.XPATH, "//*[text()='{0}']/../../../following-sibling::td[3]/div/span".format(obj)).click()
                alert = BeAlertBox(back_iframe=False)
                msg = alert.get_msg()
                if alert.title_contains("操作成功"):
                    log.info("{0}已经设置为默认模版".format(obj))
    ...
    def data_clear(self, obj, fuzzy_match=False):
        ...
        self.browser.find_element(By.XPATH, "//*[@name='templateName']/preceding-sibling::input[1]").send_keys(obj)
        self.browser.find_element(By.XPATH, "//*[text()='查询']").click()
        page_wait()
        fuzzy_match = True if fuzzy_match == "是" else False
        if fuzzy_match:
            record_element = self.browser.find_elements(
                By.XPATH, "//*[@field='templateName']//*[starts-with(text(),'{0}')]".format(obj))
        else:
            record_element = self.browser.find_elements(
                By.XPATH, "//*[@field='templateName']//*[text(),'{0}']".format(obj))
