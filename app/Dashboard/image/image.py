...
from app.Dashboard.image.util.imageType import getImageType
...
class Image:
    ...
    def add(self, imageName, catalog, interface, imageType, dataSource, style):
        ...
        if imageName:
            ...
            self.browser.find_element(By.XPATH, "//*[@id='add-name']").send_keys(imageName)
        ...
        if imageType:
            ...
            while not imageTypeExistFlag:
                ...
                try:
                    self.browser.find_element(By.XPATH, "//*[@title='{0}']/img".format(imageType)).click()
            ...
            if imageTypeExistFlag:
                ...
                self.imageType = getImageType(imageType)
            else:
                raise NoSuchElementException("找不到图像类型: {0}".format(imageType))
    ...
    def delete(self, imageName):
        ...
        self.browser.find_element(
            By.XPATH, "//*[@id='visualImageName']/following-sibling::span/input[1]").send_keys(imageName)
        log.info("图像名称输入: {0}".format(imageName))
        # 点击查询
        self.browser.find_element(By.XPATH, "//*[@title='查询可视化图像']").click()
        page_wait(1)
        self.browser.find_element(
            By.XPATH, "//*[@field='visualImageName']//div[@data-mtips='{0}']".format(imageName)).click()
        # 点击删除
        self.browser.find_element(By.XPATH, "//*[@title='删除可视化图像']").click()
        alert = BeAlertBox(timeout=3, back_iframe=False)
        msg = alert.get_msg()
        if alert.title_contains("您确定需要删除{0}吗".format(imageName), auto_click_ok=False):
            alert.click_ok()
            toast = Toast()
            msg = toast.get_msg()
            if toast.msg_contains("删除成功"):
                log.info("删除图像【{0}】成功".format(imageName))
            else:
                log.error("删除图像【{0}】失败，失败原因{1}".format(imageName, msg))
            set_global_var("ResultMsg", msg, False)
        else:
            log.error("删除图像【{0}】失败，失败原因{1}".format(imageName, msg))
            set_global_var("ResultMsg", msg, False)

    def imageClear(self, imageName, fuzzy_match=False):
        """
        :param imageName: 图像名称
        ...
        if fuzzy_match:
            record_element = self.browser.find_elements(
                By.XPATH, "//*[@field='visualImageName']//div[starts-with(@data-mtips,'{0}')]".format(imageName))
        else:
            record_element = self.browser.find_elements(
                By.XPATH, "//*[@field='visualImageName']//div[@data-mtips='{0}']".format(imageName))
            ...
            while exist_data:
                ...
                if alert.title_contains("您确定需要删除{0}吗".format(search_result), auto_click_ok=False):
                    ...
                    if toast.exist:
                        ...
                        record_element = self.browser.find_elements(
                            By.XPATH,
                            "//*[@field='visualImageName']//div[starts-with(@data-mtips,'{0}')]".format(
                                imageName))
                        if len(record_element) > 0:
                            exist_data = True
                        else:
