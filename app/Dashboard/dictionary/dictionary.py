...
class Dictionary:
    ...
    def update(self, dictName, dictionary):
        """
        :param dictName: 字典名
        ...
        self.browser.find_element(By.XPATH, "//*[@id='dictName']/following-sibling::span/input[1]").send_keys(dictName)
        log.info("字典名称输入: {0}".format(dictName))
        self.browser.find_element(By.XPATH, "//*[@title='查询字典']").click()
        page_wait(3)
        log.info("编辑字典")
        action = ActionChains(self.browser)
        element = self.browser.find_element(By.XPATH, "//*[@field='dictName']//div[text()='{0}']".format(dictName))
        row = self.browser.find_element(By.XPATH, "//*[@field='dictName']/*[text()='{0}']/../..".format(dictName))
    ...
    def delete(self, dictName):
        ...
        self.browser.find_element(By.XPATH, "//*[@field='dictName']//div[text()='{0}']".format(dictName)).click()
        # 点击删除
        self.browser.find_element(By.XPATH, "//*[@title='删除字典']").click()
        log.info("删除字典")
        alert = BeAlertBox(timeout=3, back_iframe=False)
        msg = alert.get_msg()
        if alert.title_contains("您确定需要删除{0}吗".format(dictName), auto_click_ok=False):
            alert.click_ok()
            toast = Toast()
            msg = toast.get_msg()
            if toast.msg_contains("删除成功"):
                log.info("删除字典图【{0}】成功".format(dictName))
            else:
                log.info("删字典【{0}】失败，失败原因: {1}".format(dictName, msg))
            set_global_var("ResultMsg", msg, False)
        else:
            log.error("删除字典【{0}】失败，失败原因{1}".format(dictName, msg))
            set_global_var("ResultMsg", msg, False)

    def clear_dict(self, dictNameKw, fuzzy_match=False):
        """
        :param dictNameKw: 字典名，关键字
        :param fuzzy_match: 模糊匹配
        """
        self.browser.find_element(By.XPATH, "//*[@id='dictName']/following-sibling::span/input[1]").clear()
        self.browser.find_element(By.XPATH, "//*[@id='dictName']/following-sibling::span/input[1]").send_keys(dictNameKw)
        self.browser.find_element(By.XPATH, "//*[@title='查询字典']").click()
        page_wait(3)
        fuzzy_match = True if fuzzy_match == "是" else False
        if fuzzy_match:
            record_element = self.browser.find_elements(
                By.XPATH, "//*[@field='dictName']//div[starts-with(text(),'{0}')]".format(dictNameKw))
        else:
            record_element = self.browser.find_elements(
                By.XPATH, "//*[@field='dictName']//div[text()='{0}']".format(dictNameKw))
            ...
            while exist_data:
                ...
                if alert.title_contains("您确定需要删除{0}吗".format(search_result), auto_click_ok=False):
                    ...
                    if toast.exist:
                        ...
                        record_element = self.browser.find_elements(
                            By.XPATH,
                            "//*[@field='dictName']//div[starts-with(text(),'{0}')]".format(dictNameKw))
                        if len(record_element) > 0:
                            exist_data = True
                        else:
